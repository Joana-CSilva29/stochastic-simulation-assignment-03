...
def plot_combined_radial_distribution_gradients(all_positions_data, num_charges_list, radius):
    ...
    for idx, num_charges in enumerate(num_charges_list):
        ...
        counts, bin_edges = np.histogram(radial_distances, bins=1000, range=[0, radius], density=True)
        norm = LogNorm(vmin=counts[counts > 0].min(), vmax=counts.max())
        cmap = create_custom_colormap(plt.cm.viridis, start=0, stop=0.85)
    ...
    plt.savefig(f"combined_rdg_{initial_temp}_{max_step}_{cooling_parameter}_{num_charges}_{num_simulations_per_charge}.png", dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    # num_charges_list = [11, 12]  # Example list of different numbers of charges
    num_simulations_per_charge = 30
