...
def simulate_and_visualize(initial_particles, radius, initial_temp, cooling_function, max_step, tolerance, max_consecutive_iterations, cooling_parameter, boundary_condition, reference_energy, adaptive_step_size, sequential):
...
def main():
    ...
    adaptive_options = [True, False] # [True, False] if want to compare adaptive vs fixed stepsize
    sequential = [True, False] # [True, False] if want to compare sequential vs non-sequential
    ...
    for cooling_function, cooling_parameter in cooling_schedules:
        for seq in sequential:
            neighbouring_label = "seq" if seq else "non-seq"
            ...
            for adaptive_step in range(len(adaptive_options)):
                ...
                # Labeling and storing results
                label = f"{format_function_name(cooling_function)} ({cooling_parameter}) - {neighbouring_label} - {step_size_label}"
                mean_label = label + " - Mean"
                std_label = label + " - STD"
                min_energy_label = label + " - Min Energy"
                ...
                # Plotting
                plot_label = f"{label} - Final Energy: {np.mean(energy_mins_per_run):.2f}"
